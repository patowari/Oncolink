...
const Header: React.FC<HeaderProps> = ({ onRegisterClick, onLogoClick, isRegisterPage }) => {
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center border-b border-gray-200 py-2">
          <div className="flex-shrink-0">
            <button onClick={onLogoClick} className="text-3xl font-bold text-[#003366] focus:outline-none">
              ONCO<span className="font-light">LINK</span>
            </button>
          </div>
          <div className="hidden lg:flex items-center space-x-4">
            {!isRegisterPage && (
                <button onClick={onRegisterClick} className="bg-[#007ab3] text-white px-4 py-2 rounded-md text-sm font-semibold hover:bg-[#005f8a] transition-colors">
      ...
      {isMenuOpen && !isRegisterPage && (
        <div className="lg:hidden bg-white border-t border-gray-200">
          <div className="p-4 flex flex-col space-y-2">
            ...
            <button onClick={onRegisterClick} className="w-full text-center bg-[#007ab3] text-white px-4 py-2 rounded-md text-sm font-semibold hover:bg-[#005f8a] transition-colors mt-1">
              Register for OncoLink
            </button>
          </div>
        </div>
